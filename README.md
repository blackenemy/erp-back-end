...
- **Docker Compose** — การจัดเตรียม container หลายตัว
...
- ✅ Lint pass (formatting ถูกต้อง)
- ✅ Tests pass (53/53 ผ่าน)
- 🟢 PR สามารถ merge ได้

### ❌ ผลลัพธ์เมื่อล้มเหลว

- ❌ Lint fail → ต้องแก้ไขรูปแบบ
- ❌ Tests fail → ต้องแก้ไข code
- 🔴 PR merge button ปิด (blocked)

### 🔐 Branch Protection

ต้องการ prevent direct commits ไปยัง `main` branch:

1. ไปที่ GitHub **Settings → Branches**
2. เลือก `main` branch
3. เปิด **Require status checks to pass before merging**
4. เลือก `ci / Build, Lint & Test`
5. บันทึก

📖 ดูรายละเอียด: `.github/BRANCH_PROTECTION.md`
...
# รัน tests (Release mode เหมือน CI)
dotnet test MiniPricingPlatform.slnx --configuration Release

# ถ้า pass ทั้งคู่ ให้ push ได้
git push origin feature-branch

# Create PR และรอให้ CI ผ่าน
# → Merge button จะปลดล็อก 🟢
```
...
- **version**: pin major.minor.patch ที่กำหนด
